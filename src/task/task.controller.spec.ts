...
describe('CatsController Methods', () => {
  ...
  beforeAll(async () => {
    ...
    const app: TestingModule = await Test.createTestingModule({
      ...
      providers: [TaskService, { provide: getModelToken(Task.name), useValue: taskModel }],
    }).compile();
  ...
  describe('Post Task', () => {
    ...
    it('should return (Bad Request - 400) exception', async () => {
      ...
      //create duplicate
      await expect(taskController.createTask(TaskDTOStub())).rejects.toThrow(HttpException);
    });
  });
  ...
  describe('Get Task By Id', () => {
    ...
    it('should return Not Found 404', async () => {
      await expect(taskController.findById(TaskDTOStub()._id)).rejects.toThrow(HttpException);
    });
  });
  ...
  describe('Update Task', () => {
    it('should return the updated object', async () => {
      await new taskModel(TaskDTOStub()).save();
      const updatedTask = await taskController.update(TaskDTOStub()._id, TaskDTOStubUpdated());
      expect(updatedTask.name).toBe(TaskDTOStubUpdated().name);
    });
    it('should return Not Found 404', async () => {
      await expect(taskController.update(TaskDTOStub()._id, TaskDTOStubUpdated())).rejects.toThrow(
        HttpException,
      );
    });
  });
